...
with t2:
    ...
    pclassval = st.radio("Which Class was the passenger in?", pclass_labels)
    
    sex_labels = ('female', 'male')
    sexval = st.radio("Sex of Passenger?", sex_labels)
    ...
    cabinval = st.radio("Which Cabin Set was the passenger in?", cabin_labels)
    
    embark_labels = ('Cherbourg', 'Queenstown', 'Southampton', 'Dont Know')
    embarks = ('C', 'Q', 'S', np.nan)
    embarkmap = dict(zip(embark_labels, embarks))
    embarkval = st.selectbox("Where did the passenger embark from?", embark_labels)
